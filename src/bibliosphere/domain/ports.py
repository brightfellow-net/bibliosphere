...
class BibliographyRepository(Protocol):
    ...
    # Unlike add_item, does not commit on its own — lets AddBibliography create the
    # initial copies inside the same UnitOfWork block as the bibliography itself.
...
class LoanRepository(Protocol):
    ...
    # True if the member has ever had a loan, open or returned — same rationale as
    # has_any_loan_for_item, but for deleting a member instead of removing an item.
    # count_open_loans_for_member alone isn't enough: it only covers open loans, not
    # returned ones that would still violate the loans.member_id NOT NULL FK.
