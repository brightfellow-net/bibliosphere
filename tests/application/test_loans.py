...
from bibliosphere.domain.exceptions import ItemNotAvailable, LoanLimitExceeded
...
def test_checkout_item_success(bibliography_repo, author_repo, member_repo, loan_repo):
    bibliography = _make_bibliography_with_items(bibliography_repo, author_repo)
...
def test_checkout_item_raises_when_none_available(bibliography_repo, author_repo, member_repo, loan_repo):
    bibliography = _make_bibliography_with_items(bibliography_repo, author_repo, n_items=1)
    member = CreateMember(member_repo).execute("alice", "Alice", "pw", Role.PATRON)
...
def test_checkout_item_enforces_loan_limit(bibliography_repo, author_repo, member_repo, loan_repo):
    member = CreateMember(member_repo).execute("alice", "Alice", "pw", Role.PATRON)
...
def test_return_item_frees_it_for_checkout(bibliography_repo, author_repo, member_repo, loan_repo):
    bibliography = _make_bibliography_with_items(bibliography_repo, author_repo, n_items=1)
    member = CreateMember(member_repo).execute("alice", "Alice", "pw", Role.PATRON)
    other = CreateMember(member_repo).execute("bob", "Bob", "pw", Role.PATRON)
...
def test_list_member_loans_only_open(bibliography_repo, author_repo, member_repo, loan_repo):
